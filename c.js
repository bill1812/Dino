...
function savePageLaterClick() {
  ...
  // savePageForLater will eventually trigger a call to setAutoFetchState() when it completes.
}

function cancelSavePageClick() {
  errorPageController.cancelSavePage();

  // setAutoFetchState is not called in response to cancelSavePage(), so do it now.
...
function getSuggestedContentDiv(item, index) {
  ...
  // TODO(carlosk): Present |snippet_base64| when that content becomes available.
  var thumbnail = '';
...
function offlineContentAvailable(isShown, suggestions) {
  ...
  // Sets textual web content using |textContent| to make sure it's handled as plain text.
  for (var index = 0; index < suggestions.length; index++) {
    document.getElementById(`offline-content-suggestion-title-${index}`).textContent =
      decodeUTF16Base64ToString(suggestions[index].title_base64);
    document.getElementById(`offline-content-suggestion-attribution-${index}`).textContent =
...
function onDocumentLoadOrUpdate() {
  ...
  // If offline content suggestions will be visible, the usual buttons will not be presented.
  ...
  controlButtonDiv.hidden = offlineContentVisible ||
    !(reloadButtonVisible || downloadButtonVisible || attemptAutoFetch || cacheButtonVisible);
}
